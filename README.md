...
│   ├── legal-batch/SKILL.md        # Multi-contract parallel review
...
├── scripts/
│   ├── generate_legal_pdf.py       # PDF generation (ReportLab) — xml-escape hardened
│   └── extract_review_json.py      # Parses CONTRACT-REVIEW-*.md → JSON for PDF builder
├── evo-metaclaw/                   # EvoMetaClaw meta-learning layer
│   ├── config.yml                  # Mode, niches, scheduler, circuit breaker
│   ├── legal-orchestration-genome.md # Evolved skill router + matrix thought
│   ├── population.json             # 14 skill genomes (epoch 0)
│   ├── memory/                     # Seeded domain knowledge + session signals
│   ├── matrix_thoughts.json        # 4D tensor analysis log
│   └── circuit_breaker_log.json    # Breaker events + interventions
├── .github/
│   └── workflows/
│       └── docker.yml              # Multi-arch Docker build → GHCR (amd64 + arm64)
├── Dockerfile                      # python:3.12-slim, non-root, ARM64-ready
├── templates/
...
The image rebuilds automatically on every push to `main` via GitHub Actions.
