...
description: "Converts the most recent contract review analysis into a professional PDF report with score gauge, risk bar chart, clause-by-clause analysis, and prioritized action items. Requires Python and reportlab (pip install reportlab)."
command: /legal report-pdf
---
...
1. `~/.claude/skills/legal/scripts/generate_legal_pdf.py` (installed location)
2. `[working directory]/scripts/generate_legal_pdf.py`
3. `[working directory]/ai-legal-claude/scripts/generate_legal_pdf.py`
...
```bash
python3 [script_path] "[analysis_file_path]" CONTRACT-REVIEW-REPORT.pdf
```
