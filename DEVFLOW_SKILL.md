...
| Project type | Indicators | Optimal free tier |
|---|---|---|
| Static site / JAMstack | `index.html`, no server | **Netlify** |
| Python API / agent (serverless) | `vercel.json` + `api/*.py` | **Vercel** (60 s timeout) |
| Python + Docker (any arch) | `Dockerfile` | **Railway** or **Render** |
| Docker ARM64 multi-arch | `Dockerfile` + `linux/arm64` | **Fly.io** (native ARM) |
| CI container registry | `.github/workflows/*.yml` | **GHCR** (auto on push) |
| Node.js / Next.js | `next.config.*`, `package.json` | **Vercel** |

**Detection order (first match wins):**
1. `Dockerfile` + `.github/workflows/` → **GHCR** via GitHub Actions
2. `vercel.json` present → **Vercel**
3. `netlify.toml` present → **Netlify**
4. `fly.toml` present → **Fly.io**
5. `railway.json` / `railway.toml` present → **Railway**
6. No config → GitHub Pages

**GitHub Actions → GHCR (Docker multi-arch):**
```bash
gh workflow run ci.yml
gh run list --limit 3
# Image: ghcr.io/<owner>/<repo>:latest (amd64 + arm64)
```
...
- Service Worker registered · PWA manifest valid · No mixed-content warnings

*Docker image (GHCR):*
- `gh run list --limit 1` → status: completed ✅
- `docker pull ghcr.io/<owner>/<repo>:latest && docker run --rm ... --help`
